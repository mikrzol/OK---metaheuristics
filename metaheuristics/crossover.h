...
int PMX_resolve_conflict(int conflict_int, Specimen* p1, Specimen* p2, int& larger, int& smaller, vector<std::unordered_map<int, int>> maps, int& which_map) {
    for(int c = 0; c < p1->S.size(); c++) {
        // this position will be between smaller and larger
        if(c >= smaller && c <= larger) {
            if(p1->S[c] == conflict_int) {
                ...
                auto p_to_conflict_it = maps[0].find(paired_to_conflict);
                // down - it causes conflict
                if(p_to_conflict_it != maps[0].end()) {
...
void PMX_conflict_solver(Specimen* curr_parent, Specimen* other_parent, int& larger, int& smaller, vector<std::unordered_map<int, int>> maps, int& which_map) {
    ...
    for(int i : conflict_idxes) {
        ...
        int correct_int = PMX_resolve_conflict(conflict_int, curr_parent, other_parent, larger, smaller, maps, which_map);
